...
def perform_its_analysis(data, crime_type):
    ...
    # Prepare data
    ts_data = data[['Date', crime_type]].rename(columns={'Date': 'ds', crime_type: 'y'})
    
    # Define the model with the intervention
    model = DLT(response_col='y', date_col='ds', seasonality=52, estimator='stan-map', regression_penalty='lasso', regressor_col=['intervention'])
